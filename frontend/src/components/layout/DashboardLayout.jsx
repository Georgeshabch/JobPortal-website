...
function DashboardLayout({ activeMenu , children }) {
  ...
  const [sidebarOpen, setSidarOpen] = useState(false);
  const [activeNavItem, setActiveNavItem] = useState(activeMenu || "dashboard");
  const [profileDropdowunOpen, setProfileDropdownOpen] = useState(false);
  const [isMobile, setIsMobile] = useState(false);

  // Hamdle responsive behavior
  useEffect(() => {
    const handleResize = () => {
  ...
  const toggleSilder = () => {
    setSidarOpen(!sidebarOpen);
  };

  const sildebarCollapsed = !isMobile && false;
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      >
        ...
        <div className="absolute bottom-4 left-4 right-4">
          ...
          >
            ...
            {!sildebarCollapsed && <span className="ml-3">Logout</span>}
          </button>
        </div>
      </div>
      ...
      {isMobile && sidebarOpen && (
        ...
        onClick={() => setSidarOpen(false)} />
      )}

      {/* Main content */}
      <div className={`flex-1 flex flex-col transition-all duration-300 ${
        isMobile ? "ml-0" : sildebarCollapsed ? "ml-16" : "ml-64" }`}>
          ...
          <header className="bg-white/80 backdrop-blur-sm border-b border-gray-200 h-16 flex items-center justify-between px-6 sticky top-0 z-30">
            ...
            <div className="flex items-center space-x-3">
              {/* Profile dropdown */}
              <ProfileDropdown
              isOpen={profileDropdowunOpen}
              onToggle={(e) => {
                e.stopPropagation();
                setProfileDropdownOpen(!profileDropdowunOpen);
              }}
              avatar={user?.avatar || ""}
