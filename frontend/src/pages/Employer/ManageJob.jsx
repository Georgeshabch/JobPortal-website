...
function ManageJob() {
  ...
  const filteredAndSortedJobs = useMemo(() => {
    let filtered = jobs.filter((job) => {
      const matchesSearch =
        job.title.toLowerCase().includes(searchTrem.toLowerCase()) ||
        job.company.toLowerCase().includes(searchTrem.toLowerCase());
    ...
    return filtered;
  }, [jobs, searchTrem, statusFilter, sortField, sortDirection]);

  // Pagination
  const totalPages = Math.ceil(filteredAndSortedJobs.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const pageinatedJobs = filteredAndSortedJobs.slice(
    startIndex,
  ...
  return (
    <DashboradLayout activeMenu="manage-jobs">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-8">
            <div className="flex flex-row items-center justify-between">
              ...
              <button
                className="inline-flex items-center px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 font-semibold rounded-xl text-white shadow-lg shadow-blue-500/25 hover:shadow-xl hover:shadow-blue-500/30 transition-all duration-300 transform hover:-translate-y-0.5 whitespace-nowrap"
          ...
          {/* Filter */}
          <div className="bg-white/100 backdrop-blur-sm rounded-2xl shadow-xl shadow-black/5 border border-white/20 p-6 mb-8">
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <div className="sm:w-48">
                <select
                  ...
                  className="block w-full pr-4 py-2 text-sm border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-0 transition-all duration-200 "
                >
                  <option value="All">All Status</option>
                  <option value="Active">Active</option>
                  <option value="Cloesed">CLosed</option>
                </select>
              </div>
            </div>
            ...
            <div className="my-4">
              <p className="text-sm text-gray-600">
                Showing {pageinatedJobs.length} of{" "}
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-2xl border border-white/20 overflow-hidden">
              {filteredAndSortedJobs.length === 0 && !isLoading ? (
                ...
                <div className="w-[75vw] md:w-full overflow-auto scrollbar-thin scroll-thumb-gray-300 scroll-track-gray-100">
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {isLoading
                        ...
                        : pageinatedJobs.map((job) => (
                            <tr
                              key={job.id}
                              className="hover:bg-blue-50/50 transition-all duration-200 border-b border-gray-100/60"
                            >
                              ...
                              <td className="px-6 py-5 whitespace-nowrap min-w-[130px] sm:min-w-0">
                                <button
                                  className="flex items-center text-sm text-blue-600 hover:text-blue-700 transition-colors duration-200 hover:bg-blue-50 px-2 py-1 rounded-lg"
                                  onClick={() =>
                                    navigate("/applicants", {
                                      state: { jobId: job.id },
                                    })
                              ...
                              <td className="px-6 py-5 text-sm font-medium whitespace-nowrap min-w-[180px] sm:min-w-0">
                                <div className="flex space-x-2">
                                  <button
                                  ...
                                  {job.status === "Active" ? (
                                    <button
                                      ...
                                      className="flex items-center gap-2 text-xs text-orange-600 hover:text-orange-800 p-2 rounded-lg hover:bg-orange-50 transition-all duration-200"
                                    >
                                      <X className="w-4 h-4" />
                                    ...
                                    <button
                                      onClick={() => handleStatusChange(job.id)}
                                      className="flex items-center gap-2 text-xs text-green-600 hover:text-green-800 p-2 rounded-lg hover:bg-green-50 transition-all duration-200"
                                    >
