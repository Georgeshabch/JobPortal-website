...
function Header() {
  ...
  return (
    <MotionHeader
      ...
      className="fixed top-0 left-0 right-0 bg-white/95 z-50 backdrop-blur-md shadow-sm border-b border-gray-100"
    >
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-3">
            {isAuthenticated ? (
              <div className="flex items-center space-x-3">
                <span className="text-gray-700"><span className="font-semibold">Welcome{" "}</span> {user?.name}</span>
                <a
                  href={
                    user?.role === "employer"
                      ? "/employer-dashboard"
              ...
              <>
                <a
                  href="/login"
                  className="text-gray-600 hover:text-gray-800 transition-colors font-medium px-4 py-2 rounded-lg hover:bg-gray-50"
                >
                  Login
                </a>
                <a
